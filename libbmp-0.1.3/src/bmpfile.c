...
bmp_malloc_pixels(bmpfile_t *bmp)
{
  ...
  bmp->pixels = malloc(sizeof(bw_pixel_t *) * bmp->dib.width);
  for (i = 0; i < bmp->dib.width; ++i) {
	bmp->pixels[i] = malloc(sizeof(bw_pixel_t) * bmp->dib.height);
    for (j = 0; j < bmp->dib.height; ++j) {
		bmp->pixels[i][j].uniColour = 255;
    }
  }
}
...
bool
bmp_set_pixel(bmpfile_t *bmp, uint32_t x, uint32_t y, bw_pixel_t pixel)
{
  if ((x >= bmp->dib.width) || (y >= bmp->dib.height))
    return FALSE;
...
static int
find_closest_color(bmpfile_t *bmp, bw_pixel_t pixel)
{
  int i, best = 0;
  ...
  for (i = 0; i < bmp->dib.ncolors; ++i) {
    ...
    int temp_match = INT_SQUARE(color.red - pixel.uniColour) +
      INT_SQUARE(color.green - pixel.uniColour) +
      INT_SQUARE(color.blue - pixel.uniColour);
